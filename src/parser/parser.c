...
t_simple_cmds *initialize_cmd(t_parser_tools *parser_tools)
{
    ...
    str = hb_calloc(arg_size+1,sizeof(char *));
    if(!str)
    ...
    while(arg_size>0)
    {
        if(tmp->str)
        {
            str[i++] = hb_strdup(tmp->str);
            hb_lexerdelone(&parser_tools->lexer_list,tmp->i);
            tmp = parser_tools->lexer_list;
        }
        arg_size--;
    }
...
int handle_pipe_errors(t_tools *tools,t_tokens token)
{
    if(token == PIPE)
    {
        parser_double_token_error(tools,tools->lexer_list,token);
        return (EXIT_FAILURE);
    }
    if(!tools->lexer_list)
    {
...
int parser(t_tools *tools)
{
    ...
    while(tools->lexer_list)
    {
        if(tools->lexer_list && tools->lexer_list->token ==PIPE)
            hb_lexerdelone(&tools->lexer_list,tools->lexer_list->i);
        ...
        else hb_simple_cmdsadd_back(&tools->simple_cmds,node);
