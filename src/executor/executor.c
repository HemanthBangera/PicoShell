...
int hb_fork(t_tools *tools,int end[2],int fd_in,t_simple_cmds *cmd)
{
	static int i=0;
	if(tools->reset)
	{
...
int executor(t_tools *tools)
{
	...
	while(tools->simple_cmds)
	{
		...
		hb_fork(tools,end,fd_in,tools->simple_cmds);
		close(end[1]);
	...
	tools->simple_cmds = hb_simple_cmdsfirst(tools->simple_cmds);
	return (0);
}