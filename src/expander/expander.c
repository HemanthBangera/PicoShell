...
int loop_if_dollar_sign(t_tools *tools,char *str,char **tmp,int j)
{
    ...
    while(tools->envp[k])
    {
        if(hb_strncmp(str+j+1,tools->envp[k],equal_sign(tools->envp[k])-1)==0
        && after_dol_length(str,j)-j == (int)equal_sign(tools->envp[k]))
        {
            tmp2 = hb_strdup(tools->envp[k]+equal_sign(tools->envp[k]));
            tmp3 = hb_strjoin(*tmp,tmp2);
            free(*tmp);
            *tmp = tmp3;
            free(tmp2);
...
char **expander(t_tools *tools,char **str)
{
    ...
    while(str[i] != NULL)
    {
        if(str[i][dollar_sign(str[i]) - 2] != '\'' && dollar_sign(str[i])
        ...
        if(hb_strncmp(str[0],"export",6)!=0)
        {
